...
public static class Setup {
 ...
 static void Install(){
  ...
  using(var key=Registry.CurrentUser.CreateSubKey(@"Software\Microsoft\Windows\CurrentVersion\Uninstall\FlightBridge")){key.SetValue("DisplayName","Flight Bridge");key.SetValue("DisplayVersion","0.5.1");key.SetValue("Publisher","Denis Ugarov");key.SetValue("InstallLocation",Folder);key.SetValue("DisplayIcon",Path.Combine(Folder,"FlightBridge.exe"));key.SetValue("UninstallString","\""+uninstaller+"\" /uninstall");key.SetValue("NoModify",1);key.SetValue("NoRepair",1);}
 }
